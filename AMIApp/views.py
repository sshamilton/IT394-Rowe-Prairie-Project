...
def roomRecord(request, rooms_id):
    ...
    inspections_list = Inspections.objects.filter(
        room=rooms_id).order_by('-Date')
    passRate = '100'
    ...
    context = {'inspections_list': inspections_list,
               'room': room, 'passRate': passRate}
                    ...
                    if (len(inspections_list) > 0):
                        ...
                        for inspection in inspections_list:
                            if (inspection.Pass == True):
                                passCount = passCount + 1
                        passRate = str(
                            round(((passCount / numInspections) * 100), 2))
